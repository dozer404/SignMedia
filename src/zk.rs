...
impl<F: PrimeField> ConstraintSynthesizer<F> for ClipInclusionCircuit<F> {
    fn generate_constraints(self, cs: ConstraintSystemRef<F>) -> Result<(), SynthesisError> {
        let preimage_val =
            cs.new_witness_variable(|| self.preimage.ok_or(SynthesisError::AssignmentMissing))?;
        let hash_val =
...
pub fn generate_zk_proof<R: RngCore + CryptoRng>(
    rng: &mut R,
    preimage: u128,
    hash: u128,
) -> Result<(ProvingKey<Bn254>, Proof<Bn254>), SynthesisError> {
